...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.recyclerList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                if (!binding.recyclerList.canScrollVertically(-1) && newState == RecyclerView.SCROLL_STATE_IDLE) {
        ...
        activityResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            Log.d("ggilggil", "activityResultLauncher")
            if (it.resultCode == RESULT_OK) {
                val itemIndex = it.data?.getIntExtra("itemIndex",0) as Int
                Log.d("ggilggil", "itemIndex = $itemIndex")
                val likePressed = it.data?.getBooleanExtra("likePressed",false) as Boolean
                Log.d("ggilggil", "likePressed = $likePressed")

                if(likePressed) {
                    if(!dataList[itemIndex].likePressed) {
                        dataList[itemIndex].likePressed = true
                        dataList[itemIndex].likes += 1
                    }
                }else {
                    if(dataList[itemIndex].likePressed) {
                        dataList[itemIndex].likePressed = false
                        dataList[itemIndex].likes -= 1
                    }
                }
    ...
    private fun setupRecyclerView() {
        adapter = MyAdapter(dataList, ::handleItemClick, ::handleItemLongClick) // 멤버 참조
    ...
    private fun handleItemClick(item: MyItem, position: Int) {
        val intent = Intent(this, DetailActivity::class.java).apply {
            putExtra(Const.ITEM_INDEX, position)
            putExtra(Const.ITEM_OBJECT, item)
        }
        activityResultLauncher.launch(intent)
    } // 1번 아이템에 대해서 좋아요 눌렸어 안눌렸어에 대한 답장을 듣기 위해 activityResultLauncher 사용

    private fun handleItemLongClick(position: Int): Boolean {
        val ad = AlertDialog.Builder(this)
        ad.setIcon(R.drawable.replies)
        ad.setTitle("상품 삭제")
...
//        alertDialog.setIcon(R.drawable.replies)
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            AlertDialog.Builder(this)
                .setIcon(R.drawable.replies)
                .setTitle("종료")
                .setMessage("정말 종료하시겠습니까?")
                .setPositiveButton(
                    "확인"
                ) { dialog: DialogInterface?, which: Int ->
                    finish() // Activity를 종료하는 커스텀 동작
                }
